...
parseInput =
    String.lines
        >> List.map (P.run gameParser >> Result.toMaybe)
...
roundsParser : List Round -> Parser (P.Step (List Round) (List Round))
roundsParser rounds =
    P.oneOf
...
solvePart2 =
    let
        minDiceRequired : Game -> Dict Color Int
