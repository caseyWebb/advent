        ...
        (\acc ->
            P.oneOf
                [ P.oneOf
                    ...
                    , P.const "|" Vertical
                    , P.const "7" TopRight
                    , P.const "J" BottomRight
                    , P.const "L" BottomLeft
                    , P.const "F" TopLeft
                    , P.const "S" Start
                    ]
                    |> P.map (\pipe -> P.Loop (Just pipe :: acc))
                , P.const "." (P.Loop (Nothing :: acc))
                , P.succeed (P.Done (List.reverse acc))
                ]
        )


parseInput : String -> Matrix (Maybe Pipe)
parseInput =
    String.trim
        >> String.lines
        >> List.map
